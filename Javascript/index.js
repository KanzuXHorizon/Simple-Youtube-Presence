...
var thisinterval = '';
    ...
    document.querySelector("#movie_player > div.html5-video-container > video").addEventListener('mouseover',async function() {
...
setInterval(async function() {
    ...
    async function send(Author, Thumbnail, Image, Url, Title, Now, Duration) {
        await fetch(`http://localhost:4400/?author=${encodeURIComponent(Author)}&thumbnail=${encodeURIComponent(Thumbnail)}&img=${encodeURIComponent(Image)}&url=${encodeURIComponent(Url)}&title=${encodeURIComponent(Title)}&time=${Duration}&end=${Now}&StartTime=${StartTime}`)
    }
    ...
    async function getData() {
          ...
          for (let i of dt) {
              if (i.compactVideoRenderer != undefined && i.compactVideoRenderer.videoId != undefined) {
                  if (i.compactVideoRenderer.videoId == (vid_obj.thumbnailUrl[0]).split('/')[4]) {
