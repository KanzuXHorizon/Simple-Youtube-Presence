...
(function () {
  ...
  async function getData() {
    ...
    for (let i of dt) {
      if (
        i.compactVideoRenderer != undefined &&
        i.compactVideoRenderer.videoId != undefined
      ) {
        if (
          i.compactVideoRenderer.videoId ==
          vid_obj.thumbnailUrl[0].split("/")[4]
        ) {
          author_img =
            i.thumbnail != undefined
              ? i.thumbnail.thumbnails
              : i.compactVideoRenderer.channelThumbnail.thumbnails[0].url;
          break;
        }
      }
    }
    ...
    if (author_img == undefined || author_img == "") {
      ...
      if (
        document.querySelector("#owner > ytd-video-owner-renderer > a") !=
          undefined &&
        document.querySelector("#owner > ytd-video-owner-renderer > a")
          .childNodes[0] != undefined &&
        document.querySelector("#owner > ytd-video-owner-renderer > a")
          .childNodes[0].childNodes[2] != undefined &&
        document.querySelector("#owner > ytd-video-owner-renderer > a")
          .childNodes[0].childNodes[2].src
      ) {
        author_img = document.querySelector(
